...
| `docs/DASHBOARD.md` | The plant/data mapping table still matches what `dashboard.py`'s `plantSvg` actually draws, and `build_garden_rows`' row shape still feeds it — the whole point of that view is that nothing in it is invented, so a visual tweak that stops matching the table is a doc bug. Its "When the page stops being live" table also still lists exactly the reason strings `refresh` passes to `markStale`, and its "Latest session" section still states the same window `state.session_stats()`/`SESSION_GAP_SECONDS` actually compute — the panel's whole point is that it names its window instead of implying one |
