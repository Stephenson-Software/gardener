...
`~/a-private-repo-2/CLAUDE.md`'s documented `git worktree` corruption incident).
...
way around that here. What `~/a-private-repo-2/bin/devsrv` gives you instead:
the command is *registered* so it's visible, restartable without
...
unrelated low-stakes repo — deliberately not `Stephenson-Software/a-private-repo`,
`dmccoystephenson/a-private-repo-2`, or `dmccoystephenson/gardener`, all still
