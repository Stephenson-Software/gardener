...
Mirrors `Stephenson-Software/gateway`'s `monitoring/notify-discord.sh`
...
# `Stephenson-Software/gateway`'s monitoring scripts already use
# (health-monitor.sh's recovery/failure alerts); WARNING matches that same
# repo's cert-check.sh "expiring soon" alert color. INFO has no gateway
# precedent (gateway's alerts are binary healthy/unhealthy) — Discord's own
...
def default_webhook_config_path() -> Path:
    ...
    an env var every invocation isn't practical, mirroring gateway's own
...
class DiscordNotifier(Notifier):
    ...
    convention. Mirrors gateway's `notify-discord.sh`: silent no-op if no
    ...
    def notify(self, title: str, message: str, level: Level = Level.INFO) -> None:
        ...
        request = urllib.request.Request(
            ...
            # User-Agent outright — confirmed directly: the exact same POST
            # succeeds with any browser/curl-like UA. See gardener PR for the
            # real webhook test that caught this.
