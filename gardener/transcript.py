...
2. Cross-checked against a real, already-existing gardener dispatch's own
   ...
   `Stephenson-Software/a-private-repo` earlier the same night: cwd
   `/home/userland/.cache/gardener/repos/Stephenson-Software__a-private-repo`
   (gardener's own `repo.replace("/", "__")` cache-dir naming, see
   `cli.py`'s `clone_or_refresh_target_repo`) produced
   `-home-userland--cache-gardener-repos-Stephenson-Software--a-private-repo` —
...
# real `Stephenson-Software/a-private-repo` transcript on 2026-07-18 (parsing each
